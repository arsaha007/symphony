...
import (
	...
	"github.com/eclipse-symphony/symphony/api/pkg/apis/v1alpha1/managers/solutionversion/metrics"
...
type SolutionVersionManager struct {
	SummaryManager
...
type SolutionVersionManagerDeploymentState struct {
	Spec  model.DeploymentSpec  `json:"spec,omitempty"`
	State model.DeploymentState `json:"state,omitempty"`
}

func (s *SolutionVersionManager) Init(context *contexts.VendorContext, config managers.ManagerConfig, providers map[string]providers.IProvider) error {
...
func (s *SolutionVersionManager) GetSummary(ctx context.Context, summaryId string, name string, namespace string) (model.SummaryResult, error) {
	return s.SummaryManager.GetSummary(ctx, fmt.Sprintf("%s-%s", "summary", summaryId), name, namespace)
}

func (s *SolutionVersionManager) DeleteSummary(ctx context.Context, summaryId string, namespace string) error {
...
func (s *SolutionVersionManager) sendHeartbeat(ctx context.Context, id string, namespace string, remove bool, stopCh chan struct{}) {
	...
	for {
		select {
		case <-ticker.C:
			log.DebugfCtx(ctx, " M (SolutionVersion): sendHeartbeat, id: %s, namespace: %s, remove:%v", id, namespace, remove)
...
func (s *SolutionVersionManager) cleanupHeartbeat(ctx context.Context, id string, namespace string, remove bool) {
	if !remove {
		return
	}

	log.DebugfCtx(ctx, " M (SolutionVersion): cleanupHeartbeat, id: %s, namespace: %s", id, namespace)
	s.VendorContext.Publish("heartbeat", v1alpha2.Event{
		Body: v1alpha2.HeartBeatData{
			JobId:     id,
...
func (s *SolutionVersionManager) Reconcile(ctx context.Context, deployment model.DeploymentSpec, remove bool, namespace string, targetName string) (model.SummarySpec, error) {
	...
	log.InfofCtx(ctx, " M (SolutionVersion): reconciling deployment.InstanceName: %s, deployment.SolutionVersionName: %s, remove: %t, namespace: %s, targetName: %s, generation: %s, jobID: %s",
		deployment.Instance.ObjectMeta.Name,
		deployment.SolutionVersionName,
		remove,
		namespace,
		targetName,
	...
	if deployment.IsInActive {
		log.InfofCtx(ctx, " M (SolutionVersion): deployment is not active, remove the deployment")
		remove = true
	}
	...
	if summaryId == "" {
		log.ErrorfCtx(ctx, " M (SolutionVersion): object GUID is null: %+v", err)
		return summary, err
	}
	...
	if err != nil {
		log.ErrorfCtx(ctx, " M (SolutionVersion): failed to save summary progress: %+v", err)
		return summary, err
	}
	defer func() {
		if r := recover(); r == nil {
	...
	componentCount := len(deployment.SolutionVersion.Spec.Components)
	...
	if err != nil {
		if remove {
			log.InfofCtx(ctx, " M (SolutionVersion): skipped failure to evaluate deployment spec: %+v", err)
		} else {
			summary.SummaryMessage = "failed to evaluate deployment spec: " + err.Error()
			log.ErrorfCtx(ctx, " M (SolutionVersion): failed to evaluate deployment spec: %+v", err)
			return summary, err
		}
	}
	...
	if err != nil {
		...
		log.ErrorfCtx(ctx, " M (SolutionVersion): failed to plan for deployment: %+v", err)
		return summary, err
	}

	col := api_utils.MergeCollection(deployment.SolutionVersion.Spec.Metadata, deployment.Instance.Spec.Metadata)
	dep := deployment
	...
	log.DebugfCtx(ctx, " M (SolutionVersion): reconcile save summary progress: start deploy, total %v deployments", summary.PlannedDeployment)
	...
	planSuccessCount := 0
	for _, step := range plan.Steps {
		log.DebugfCtx(ctx, " M (SolutionVersion): processing step with Role %s on target %s", step.Role, step.Target)
		for _, component := range step.Components {
			log.DebugfCtx(ctx, " M (SolutionVersion): processing component %s with action %s", component.Component.Name, component.Action)
		}
		...
		if previousDesiredState != nil && !remoteTarget {
			...
			if s.canSkipStep(ctx, step, step.Target, provider.(tgt.ITargetProvider), previousDesiredState.State.Components, testState) {
				...
				log.InfofCtx(ctx, " M (SolutionVersion): skipping step with role %s on target %s", step.Role, step.Target)
		...
		log.DebugfCtx(ctx, " M (SolutionVersion): applying step with Role %s on target %s", step.Role, step.Target)
		someStepsRan = true
		retryCount := 1
		...
		// 			} else {
		// 				log.ErrorfCtx(ctx, " M (SolutionVersion): failed to evaluate property: %+v", err)
		...
		if stepError != nil {
			log.ErrorfCtx(ctx, " M (SolutionVersion): failed to execute deployment step: %+v", stepError)
		...
		log.DebugfCtx(ctx, " M (SolutionVersion): reconcile save summary progress: current deployed %v out of total %v deployments", summary.CurrentDeployed, summary.PlannedDeployment)
	}
	...
	if !deployment.IsDryRun {
		if len(mergedState.TargetComponent) == 0 && remove {
			log.DebugfCtx(ctx, " M (SolutionVersion): no assigned components to manage, deleting state")
...
func (s *SolutionVersionManager) getTargetStateForStep(step model.DeploymentStep, deployment model.DeploymentSpec, previousDeploymentState *SolutionVersionManagerDeploymentState) model.TargetState {
...
func (s *SolutionVersionManager) saveSummary(ctx context.Context, objectName string, summaryId string, generation string, hash string, summary model.SummarySpec, state model.SummaryState, namespace string) error {
	...
	log.DebugfCtx(ctx, " M (SolutionVersion): saving summary, objectName: %s, summaryId: %s, state: %v, namespace: %s, jobid: %s, hash %s, targetCount %d, successCount %d",
...
func (s *SolutionVersionManager) saveSummaryProgress(ctx context.Context, objectName string, summaryId string, generation string, hash string, summary model.SummarySpec, namespace string) error {
	return s.saveSummary(ctx, objectName, summaryId, generation, hash, summary, model.SummaryStateRunning, namespace)
}

func (s *SolutionVersionManager) concludeSummary(ctx context.Context, objectName string, summaryId string, generation string, hash string, summary model.SummarySpec, namespace string) error {
	return s.saveSummary(ctx, objectName, summaryId, generation, hash, summary, model.SummaryStateDone, namespace)
}

func (s *SolutionVersionManager) canSkipStep(ctx context.Context, step model.DeploymentStep, target string, provider tgt.ITargetProvider, previousComponents []model.ComponentSpec, currentState model.DeploymentState) bool {
...
func (s *SolutionVersionManager) getWithNamespace(ctx context.Context, deployment model.DeploymentSpec, targetName string, namespace string) (model.DeploymentState, []model.ComponentSpec, error) {
	...
	defer observ_utils.CloseSpanWithError(span, &err)
	defer observ_utils.EmitUserDiagnosticsLogs(ctx, &err)
	log.InfofCtx(ctx, " M (SolutionVersion): getting deployment.InstanceName: %s, deployment.SolutionVersionName: %s, targetName: %s",
		deployment.Instance.ObjectMeta.Name,
		deployment.SolutionVersionName,
		targetName)
	...
	if err != nil {
		log.ErrorfCtx(ctx, " M (SolutionVersion): failed to create manager state for deployment: %+v", err)
		return ret, nil, err
	}
	var plan model.DeploymentPlan
...
func (s *SolutionVersionManager) Enabled() bool {
	return s.Config.Properties["poll.enabled"] == "true"
}
func (s *SolutionVersionManager) Poll() []error {
	log.InfofCtx(context.Background(), " M (SolutionVersion): Poll() called, pollEnabled=%s, currentUrl=%s, isTarget=%v, targetNames=%v",
	...
	if s.Config.Properties["poll.enabled"] == "true" && s.Context.SiteInfo.CurrentSite.BaseUrl != "" && s.IsTarget {
		log.InfofCtx(context.Background(), " M (SolutionVersion): conditions met, starting to process %d targets", len(s.TargetNames))
		for _, target := range s.TargetNames {
			namespace := s.TargetNamespace
			catalogversions, err := s.ApiClientHttp.GetCatalogVersionsWithFilter(context.Background(), "", "label", "staged_target="+target,
				s.Context.SiteInfo.CurrentSite.Username,
				s.Context.SiteInfo.CurrentSite.Password)
			if err != nil {
				return []error{err}
			}
			for _, c := range catalogversions {
				...
				if vs, ok := c.Spec.Properties["deployment"]; ok {
					...
					err = s.ApiClientHttp.ReportCatalogVersions(context.Background(),
			...
			// Report target status on every poll cycle regardless of whether catalogversions are staged
			...
			if targetReportErr != nil {
				log.WarnfCtx(context.Background(), " M (SolutionVersion): failed to report target status for target %s: %v", target, targetReportErr)
			} else {
				log.InfofCtx(context.Background(), " M (SolutionVersion): successfully reported target status for target %s in namespace %s", target, namespace)
			}
		}
	}
	return nil
...
func getCurrentApplicationScope(ctx context.Context, instance model.InstanceState, target model.TargetState) string {
	log.InfofCtx(ctx, " M (SolutionVersion): getting current application scope, instance scope: %s, target application scope: %s", instance.Spec.Scope, target.Spec.SolutionVersionScope)
	if instance.Spec.Scope == "" {
		if target.Spec.SolutionVersionScope == "" {
			return "default"
		}
		return target.Spec.SolutionVersionScope
	}
	if target.Spec.SolutionVersionScope != "" && target.Spec.SolutionVersionScope != instance.Spec.Scope {
		message := fmt.Sprintf(" M (SolutionVersion): target application scope: %s is inconsistent with instance scope: %s", target.Spec.SolutionVersionScope, instance.Spec.Scope)
